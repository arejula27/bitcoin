...
public:
    IndexBlockSim(const fs::path& parent_dir, std::unique_ptr<interfaces::Chain> chain, bool f_memory, const int blocking_height) :
        BaseIndex(std::move(chain), "sim_index", "simidx"), m_blocking_height(blocking_height)
    {
...
BOOST_FIXTURE_TEST_CASE(initial_sync_reorg, TestChain100Setup)
{
    const auto& chainman = m_node.chainman;

    // Create an index that will block during initial sync
    ...
    BOOST_REQUIRE(BuildChain(m_node, fork_point, CScript() << OP_TRUE, fork_length, fork_chain));
...
BOOST_FIXTURE_TEST_CASE(shutdown_during_initial_sync, TestChain100Setup)
{
    ...
    // Commit() refuses to persist the locator while the index is ahead of the
    // chainstate's last flushed block, so force a flush up front to let the
    // interrupted sync checkpoint its progress.
    Assert(m_node.chainman)->ActiveChainstate().ForceFlushStateToDisk();
    m_node.validation_signals->SyncWithValidationInterfaceQueue();
