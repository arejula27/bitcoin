...
public:
    ...
    virtual void FlushErrorHandler(FlushError error, std::string_view message) {}

    virtual void FatalErrorHandler(FatalError error, std::string_view message) {}
};
    ...
    void SetNotifications(std::shared_ptr<T> notifications)
    {
        ...
        btck_context_options_set_notifications(
            ...
            btck_NotificationInterfaceCallbacks{
                ...
                .flush_error = +[](void* user_data, btck_FlushError error, const char* message, size_t message_len) { (*static_cast<user_type>(user_data))->FlushErrorHandler(static_cast<FlushError>(error), {message, message_len}); },
                .fatal_error = +[](void* user_data, btck_FatalError error, const char* message, size_t message_len) { (*static_cast<user_type>(user_data))->FatalErrorHandler(static_cast<FatalError>(error), {message, message_len}); },
            });
    }
