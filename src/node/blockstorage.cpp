...
bool BlockManager::LoadBlockIndex(const std::optional<uint256>& snapshot_blockhash)
{
    ...
    if (snapshot_blockhash) {
        ...
        if (!maybe_au_data) {
            m_opts.notifications.fatalError(kernel::FatalError::ASSUMEUTXO_DATA_NOT_FOUND, {snapshot_blockhash->ToString()});
            return false;
        }
...
FlatFilePos BlockManager::FindNextBlockPos(unsigned int nAddSize, unsigned int nHeight, uint64_t nTime)
{
    ...
    if (out_of_space) {
        m_opts.notifications.fatalError(kernel::FatalError::DISK_SPACE_TOO_LOW);
        return {};
    }
...
bool BlockManager::FindUndoPos(BlockValidationState& state, int nFile, FlatFilePos& pos, unsigned int nAddSize)
{
    ...
    if (out_of_space) {
        return FatalError(m_opts.notifications, state, kernel::FatalError::DISK_SPACE_TOO_LOW);
    }
    if (bytes_allocated != 0 && IsPruneMode()) {
        m_check_for_pruning = true;
    }
...
bool BlockManager::WriteBlockUndo(const CBlockUndo& blockundo, BlockValidationState& state, CBlockIndex& block)
{
    ...
    if (block.GetUndoPos().IsNull()) {
        ...
        if (file.IsNull()) {
            ...
            return FatalError(m_opts.notifications, state, kernel::FatalError::UNDO_DATA_WRITE_FAILED);
        }
        {
        ...
        if (file.fclose() != 0) {
            ...
            return FatalError(m_opts.notifications, state, kernel::FatalError::UNDO_FILE_CLOSE_FAILED);
        }
...
FlatFilePos BlockManager::WriteBlock(const CBlock& block, int nHeight)
{
    ...
    if (file.IsNull()) {
        ...
        m_opts.notifications.fatalError(kernel::FatalError::BLOCK_WRITE_FAILED);
        return FlatFilePos();
    }
    {
        BufferedWriter fileout{file};
    ...
    if (file.fclose() != 0) {
        ...
        m_opts.notifications.fatalError(kernel::FatalError::BLOCK_FILE_CLOSE_FAILED);
        return FlatFilePos();
    }
...
void ImportBlocks(ChainstateManager& chainman, std::span<const fs::path> import_paths)
{
    ...
    if (auto result = chainman.ActivateBestChains(); !result) {
        chainman.GetNotifications().fatalError(kernel::FatalError::ACTIVATE_BEST_CHAINS_FAILED, {util::ErrorString(result).original});
    }
