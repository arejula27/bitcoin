 ...
 *
 * The index still sits in LevelDB's write buffer, so the queries are served
 * from memory rather than from a table on disk, and the lookup APIs also read
 * and deserialize the transaction from the block file, which dominates.
 */
